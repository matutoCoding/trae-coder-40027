...
from data.models import DispatchRecord
from ui.styles import AppStyles
from datetime import datetime


class DispatchLoadPage(BasePage):
    def __init__(self):
        super().__init__("发料装车")
        self.records = MockData.get_dispatch_records()
        self.record_row_map = []
        self._build_ui()
        self.refresh_all()
    ...
    def _build_ui(self):
        ...
        card, card_layout = self.create_card("罐车装卸记录")
        
        toolbar, toolbar_layout = self.create_toolbar()
        
        self.search_input = self.create_search_input("搜索单号、罐号、车牌号、客户...")
        ...
        self.filter_combo.addItems(["全部状态", "待装车", "已完成", "待复核"])
        ...
        add_btn = self.create_primary_button("+ 新增发料")
        ...
        headers = ["记录编号", "储罐", "介质", "客户", "车牌号", "发料日期", 
                   "发料量(m³)", "温度(℃)", "密度", "操作人", "状态"]
    ...
    def _rebuild_stats(self):
        ...
        total_volume = sum(r.dispatch_volume for r in self.records if r.status == "已完成")
        completed = sum(1 for r in self.records if r.status == "已完成")
        pending = sum(1 for r in self.records if r.status in ["待装车", "待复核"])
        today_count = sum(1 for r in self.records if r.dispatch_date == today)
        
        card1 = self.create_stat_card("今日发车", f"{today_count} 车", "今日装车次数", AppStyles.PRIMARY_COLOR)
        card2 = self.create_stat_card("本月累计", f"{total_volume:.1f} m³", "累计发料量", AppStyles.INFO_COLOR)
        card3 = self.create_stat_card("已完成", f"{completed} 笔", "历史记录", AppStyles.SUCCESS_COLOR)
        card4 = self.create_stat_card("待装车", f"{pending} 笔", "等待装车", AppStyles.WARNING_COLOR)
    ...
    def _on_add(self):
        dialog = DispatchDialog(self)
        if dialog.exec() == QDialog.Accepted:
            data = dialog.get_data()
            if data:
                rec = DispatchRecord(**data)
                self.records.insert(0, rec)
                self.refresh_all()
                QMessageBox.information(self, "提示", "发料记录添加成功！")
    ...
    def _on_detail(self):
        ...
        if record is None:
            ...
            return
        dialog = DispatchDetailDialog(record, self)
        dialog.exec()


class DispatchDialog(BaseDialog):
    def __init__(self, parent=None):
        super().__init__("新增发料记录", parent)
    ...
    def _build_form(self):
        ...
        self.add_field("来源储罐", self.tank_combo)
        
        self.customer_edit = QLineEdit()
        self.customer_edit.setFixedHeight(32)
        self.add_field("客户单位", self.customer_edit)
        ...
        self.add_field("发前体积", self.before_spin)
        ...
        self.add_field("发后体积", self.after_spin)
    ...
    def get_data(self):
        ...
        if tank_data is None:
            QMessageBox.warning(self, "提示", "请选择来源储罐")
            return None
        if not self.customer_edit.text().strip():
            QMessageBox.warning(self, "提示", "请输入客户单位")
            return None
        
        now = datetime.now()
        before = self.before_spin.value()
        after = self.after_spin.value()
        dispatch_vol = max(0.0, before - after)
        
        return {
            "id": f"DS-{now.strftime('%Y%m%d%H%M%S')}",
            ...
            "dispatch_date": now.strftime("%Y-%m-%d"),
            "dispatch_time": now.strftime("%H:%M"),
            "before_volume": before,
            "after_volume": after,
            "dispatch_volume": dispatch_vol,
            "before_level": 0.0,
            "after_level": 0.0,
...
class DispatchDetailDialog(QDialog):
    def __init__(self, record, parent=None):
        ...
        self.record = record
        self.setWindowTitle(f"发料详情 - {record.id}")
    ...
    def _build_ui(self):
        ...
        status_type_map = {"已完成": "normal", "待装车": "warning", "待复核": "info", "已取消": "error"}
        ...
        title_label = QLabel(f"发料记录：{self.record.id}")
        ...
        basic_items = [
            ...
            ("客户", self.record.customer),
            ("车牌号", self.record.vehicle_no),
            ("司机", self.record.driver),
            ("发料日期", f"{self.record.dispatch_date} {self.record.dispatch_time}"),
        ]
        ...
        meter_items = [
            ("发前体积", f"{self.record.before_volume:.1f} m³"),
            ("发后体积", f"{self.record.after_volume:.1f} m³"),
            ("发料量", f"{self.record.dispatch_volume:.1f} m³"),
            ("发前液位", f"{self.record.before_level:.2f} m"),
            ("发后液位", f"{self.record.after_level:.2f} m"),
